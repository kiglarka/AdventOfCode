...
// Result: multiply them
    ...
    private fun findTriplet(list: ArrayList<Int>): Int {
        // create set
        ...
        for (item in i until list.size) {
            for (anotherItem in j until list.size - 1) {
                setter.add(Pair(list.get(item), list.get(anotherItem)))
            }
        }
    ...
    fun pairToTwenty(list: ArrayList<Int>): Pair<Int, Int> {
    ...
    fun multiply(pair: Pair<Int, Int>): Int = pair.first * pair.second
