...
// QUESTION: How many passwords are valid
    ...
    fun checkPosition(password: String, index: Int, char: Char): Boolean {
        return if (password.get(index).toChar() == char) true else false
    }

    fun checkNewPolicy(item: String): Boolean {
        val list = item.split(" ")
    ...
    fun checkPassword(item: String): Boolean {
    ...
    fun countCorrect(list: ArrayList<String>): Int =
        list.count { checkPassword(it) }

    fun countNewCorrect(list: ArrayList<String>): Int =
        list.count { checkNewPolicy(it) }


    fun main() {
        val miniList = arrayListOf("6-10 p: ctpppjmdpppppp", "17-19 l: llllllllllllllllllll")
